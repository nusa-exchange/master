namespace :service do
  ...
  task :app, [:command] do |task, args|
    ...
    def start
      puts '----- Starting app -----'
      sh 'docker-compose up -d peatio barong finex-api quantex-api rango applogic envoy frontend admin assets-currency'
    end

    def stop
      puts '----- Stopping app -----'
      sh 'docker-compose rm -fs peatio barong finex-api quantex-api rango applogic envoy frontend admin assets-currency'
    end
